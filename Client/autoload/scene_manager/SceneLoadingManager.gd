...
var global_scene_map = preload("res://autoload/map/GlobalSceneMap.gd").new()
...
func put_scene_at_node(scene_name: String, node_path: String) -> Node:
	print("Adding scene", scene_name, "to node:", node_path)  # Debug log
	...
	# Check if the scene is already a child of this node to avoid re-adding it
	if parent_node.has_node(scene_name):
		print("Scene", scene_name, "is already added to node:", node_path)
		return parent_node.get_node(scene_name)
	
	# Add the scene instance to the parent node
	parent_node.add_child(scene_instance)
	print("Scene", scene_name, "added to node:", node_path)
	return scene_instance
...
func get_node_or_create(node_path: String) -> Node:
	...
	for part in path_parts:
		part = part.strip_edges()  # Ensure no spaces or incorrect characters
		if part == "":
			continue
		if current_node.has_node(part):
			current_node = current_node.get_node(part)
		else:
			print("Node ", part, " not found. Creating new node.")  # Improved debug log
			# Check if the node was just created in a previous iteration
			if current_node.has_node(part):
				current_node = current_node.get_node(part)
			else:
