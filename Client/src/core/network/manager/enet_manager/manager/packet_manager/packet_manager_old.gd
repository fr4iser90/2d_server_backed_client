# res://src/core/network/manager/packet_manager.gd (Client)
extends Node
...
# Cache the  channel map and store handlers
func cache_channel_map():
	# Clear the existing handler cache
	handler_channel_cache.clear()
	# Iterate through the channel map and cache the handlers
...
func process_packet(packet: PackedByteArray, peer_id: int):
	...
	print("Retrieved handler for channel ", channel, ":", handler, "from peer_id:", peer_id)
	...
	if not handler.has_method("handle_packet"):
		print("Handler: ", handler,  " for channel: ", channel, " is missing 'handle_packet' method, peer_id: ", peer_id)
		return
