...
func initialize():
	if is_initialized:
		...
		return
	is_initialized = true
	server_init_manager = load("res://server/preset/database_mongodb_rest/server_init_manager.gd").new()
	network_manager_loader = load("res://server/preset/database_mongodb_rest/network_manager_loader.gd").new()
