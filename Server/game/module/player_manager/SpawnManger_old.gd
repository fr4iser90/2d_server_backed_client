...
# Spawns a player in an existing instance
func spawn_player(peer_id: int, character_class: String, scene_name: String, spawn_point: String, use_last_position: bool = false):
	...
	# Request the correct instance from InstanceManager
	var instance_manager = GlobalManager.NodeManager.get_cached_node("world_manager", "instance_manager")
	var instance_key = instance_manager.assign_player_to_instance(scene_name, {
		"peer_id": peer_id,
		"character_class": character_class
	})

	if instance_key != "":
		# Retrieve the scene instance from the assigned instance
		...
		print("Error: Failed to assign player to an instance.")
