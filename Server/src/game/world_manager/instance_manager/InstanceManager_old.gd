...
@onready var instance_creation_handler = $Handler/InstanceLifecycleHandler/InstanceCreationHandler
@onready var instance_cache_handler = $Handler/InstanceLifecycleHandler/InstanceCacheHandler
@onready var instance_assignment_handler = $Handler/InstanceLifecycleHandler/InstanceAssignmentHandler
@onready var instance_destruction_handler = $Handler/InstanceLifecycleHandler/InstanceDestructionHandler
...
func get_minimal_player_data(instance_key: String) -> Array:
	...
	for player_data in instances[instance_key]["players"]:
		...
		minimal_data.append({
			...
			"character_class": character_data.get("class", {})  # Optional appearance info
		})
	return minimal_data
