...
func initialize():
	...
	# Connect to the signal emitted when the map is ready
...
func cache_channel_map():
	...
	for channel in global_channel_map.GLOBAL_CHANNEL_MAP.keys():
		var handler_name = global_channel_map.GLOBAL_CHANNEL_MAP[channel]
